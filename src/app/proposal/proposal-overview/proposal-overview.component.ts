...
export class ProposalOverviewComponent implements OnInit, OnDestroy {
  public disableVal: boolean = true;
  public userId: any;
  // ----------------------------------
  ...
  public chooseList: any = [
    { ids: 'Agreement', name: 'Agreement Id' },
    {
      ids: 'Enrollment',
      name: 'Enrollment Id',
    },
    ...
    this.proposalService.getamendmentstaticInfo().subscribe((data) => {
      ...
      if (this.ProposalId != 0 && this.ProposalId != null) {
        this.proposalService
          .getProposalCopy(this.ProposalId)
          .subscribe((data: any) => {
            this.editProposalObj = data;
            ...
            if (this.model == null || this.model.ID == undefined) {
              console.log({
                content:
                  'Proposal does not exists or you don`t have permissions to view it.',
              });
            }
  ...
  captureHRDDvalues() {
    if (this.HRDDCountries.length == 0) {
      this.proposalService.getHrdCountries().subscribe((data: any) => {
        ...
        var isHrddCountry = this.proposalService.isHRDDCountry(
          this.HRDDCountries,
          this.model.PricingCountry
        );
        var hasHrddAmendments = this.proposalService.hasHRDDAmendments(
          this.HRDDAmendments,
          this.model.Amendments
        );
        if (
          isHrddCountry &&
          hasHrddAmendments &&
          this.model.HRDDTotalValue == null &&
          this.model.HRDDMaxDiscount == null
        ) {
          // if (true) {
  ...
  initDiscountAmendmentPopUp() {
    ...
    for (var ai in this.model.Amendments) {
      ...
      var isDiscountedAmendment = this.proposalService.isDiscountedAmendment(
        this.discountAmendments,
        amendment.code
      );
      if (
        isDiscountedAmendment != null &&
        isDiscountedAmendment.length > 0 &&
        (amendment.Discount == null ||
          amendment.BeginDate == null ||
          amendment.EndDate == null)
      ) {
        if (isDiscountedAmendment[0].DiscountType == 1) {
          // equals to MonetaryCommitmentACD
        ...
        } else if (isDiscountedAmendment[0].DiscountType == 2) {
          // equals to PayAsYouGoACD
          this.DealSizeRequired = false;
          this.CommitmentConsumeRequired = false;
          this.ShowCommitToConsumeAmount = false;
          this.IsPAYG = true;
          this.DiscountTitle = 'Pay As You Go';
        } else if (isDiscountedAmendment[0].DiscountType == 3) {
          // equals to CommitmenttoConsumeACD
  ...
  proposalUpdate(event, block) {
    ...
    var obj = {
      ...
      proposalId: this.propOverView.get('proposalId').value
        ? this.propOverView.get('proposalId').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.proposalId
        : '',

      pricingCountry: this.propOverView.get('pricingCountry').value
        ? this.propOverView.get('pricingCountry').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.pricingCountry
        : '',
      // : this.editProposalObj.pricingCountry,

      enrollmentId: this.propOverView.get('enrollmentId').value
        ? this.propOverView.get('enrollmentId').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.enrollmentId
        : '',

      agreementId: this.propOverView.get('agreementId').value
        ? this.propOverView.get('agreementId').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.agreementId
        : '',
      identifier: this.propOverView.get('identifier').value
        ? this.propOverView.get('identifier').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.identifier
        : null,

      customerName: this.propOverView.get('customerName').value
        ? this.propOverView.get('customerName').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.customerName
        : null,

      dealNickname: this.propOverView.get('dealNickname').value
        ? this.propOverView.get('dealNickname').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.dealNickname
        : '',

      notes: this.propOverView.get('notes').value
        ? this.propOverView.get('notes').value
        : Object.keys(this.editProposalObj).length > 0
        ? this.editProposalObj['proposalEntity']?.notes
        : '',
  ...
  blurUpdateForm(obj, block) {
    this.proposalService.updateProposal(obj).subscribe((data: any) => {
      ...
      this.toastr.success(
        'Proposal ' + block + ' Updated Successfully',
        'Success'
      );
  ...
  loadForm() {
    ...
    if (
      this.editProposalObj['proposalEntity']?.proposalId != undefined &&
      this.editProposalObj['proposalEntity']?.proposalId != ''
    ) {
    ...
    this.propOverView = new FormGroup({
      ...
      pricingCountry: new FormControl(
        this.editProposalObj['proposalEntity']?.pricingCountry
          ? this.editProposalObj['proposalEntity']?.pricingCountry
          : null
      ),
  ...
  open() {
    this.proposalService
      .searchAmendement(
        this.propOverView.get('searchAmendment').value,
      ...
      .subscribe((data: any) => {
        ...
        modelRef.componentInstance.selectAmendement.subscribe(
          (receivedEntry) => {
            var obj1 = [
              {
                Id: receivedEntry.id,
  ...
  getVersion() {
    for (var i = 0; i < this.Amendments.length; i++) {
      this.Amendments[i].location = this.Amendments[i].fileName
        .split('(')[3]
        .split(')')[0];
      this.Amendments[i].version = this.Amendments[i].fileName
        .split('(')[5]
        .split(')')[0];
    }
    ...
    if (
      ...
      if (this.Amendments.length > 0) {
        ...
        if (this.isPricingCountry() || this.IsPricingAmendmentExists()) {
          ...
          obj['type'] = '0';
          obj['userAlias'] =
            this.editProposalObj['proposalEntity'].createdByAlias;
  ...
  showEditDocumentPopup(obj, protection) {
    ...
    window.location.href =
      'amendmentappwordservice://documentsService?aid=' +
      aid +
      '&fileName=' +
      obj.fileName +
      '&protection=' +
  ...
  // intializeTimer(FileName){
  //      fileName = FileName;
  //      timer = $interval(checkStatus(), 2000);
  // }

  //  checkStatus () {
  //     EditDocumentService.checkInProgress(fileName).then(function (data) {
  //         console.log(data);
  //         if (data && data.Status == 2) {
  //             $(".exe-loader").fadeOut("slow");
  //             $rootScope.donthide = false;
  //             if (timer) $interval.cancel(timer);
  //             if (isEdit) {
  //                 var found_amendment = $.grep($scope.model.Amendments, function (v) {
  //                     return v.Id == aid;
  //                 });
  //                 var amendmentType = (found_amendment.length >= 1) ? found_amendment[0].Type : 0;
  //                  $state.go("ctmfooter", { proposalId: $scope.model.ID, isEdit: true, aid: aid, amtype: amendmentType });
  //             }
  //             else {
  //                 $scope.model.Amendments.filter(function (p) { return p.Id == aid })[0].IsEditField = "Yes";
  //             }
  //         }
  //         //else $(".loader").show();
  //     }, function (error) {
  ...
  showEditAdditionalDocumentFields(obj, protection) {
    ...
    var aid = obj.id;
    this.proposalService
      .startEditDocument(this.ProposalId, aid)
      .subscribe((data) => {
          console.log(data);
          debugger;
        if (data && data.fileName) {
       // donthide = true;
        // $(".exe-loader").show();
        //intializeTimer(data.FileName);
        window.location.href = "amendmentappwordservice://documentsService?aid=" + aid + "&fileName=" + data.fileName + "&protection=" + protection;
        }
      });
  }
  openActiveproposal() {
